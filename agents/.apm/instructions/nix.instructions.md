...
- **Shared env vars** live in `appEnv` (defined in `default.nix`). Both the build and the devShell consume it — don't duplicate env var definitions.
- **Measure `nix develop` time** after Nix changes: `time nix develop -c echo test`. Current target: <PLACEHOLDER> cold, <PLACEHOLDER> warm.
